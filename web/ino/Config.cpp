...
void storageReset() {
  ...
  strncpy(cfg.mdns_name, "esp-device", sizeof(cfg.mdns_name) - 1);
  ...
  strncpy(cfg.ap_ssid, "", sizeof(cfg.ap_ssid) - 1);
  strncpy(cfg.ap_password, "", sizeof(cfg.ap_password) - 1);
  cfg.ap_fallback = 1;
  strncpy(cfg.ota_url, "", sizeof(cfg.ota_url) - 1);
  cfg.ota_check_interval = 0;
  cfg.wifi_retry_count = 2;
  storageSave();
}
...
bool networkListRemove(int idx) {
  if (idx < 0 || idx >= nl.count) return false;
  for (int i = idx; i < nl.count - 1; i++) {
    nl.networks[i] = nl.networks[i + 1];
  }
  nl.count--;
  networkListSave();
  return true;
